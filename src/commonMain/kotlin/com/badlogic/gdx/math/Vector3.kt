 ...
 */
class Vector3 : java.io.Serializable, com.badlogic.gdx.math.Vector<Vector3?> {
    ...
    fun setFromSpherical(azimuthalAngle: Float, polarAngle: Float): Vector3 {
        val cosPolar: Float = MathUtils.cos(polarAngle)
        val sinPolar: Float = MathUtils.sin(polarAngle)
        val cosAzim: Float = MathUtils.cos(azimuthalAngle)
        val sinAzim: Float = MathUtils.sin(azimuthalAngle)
        return this.set(cosAzim * sinPolar, sinAzim * sinPolar, cosPolar)
    }

    override fun setToRandomDirection(): Vector3 {
        val u: Float = MathUtils.random()
        val v: Float = MathUtils.random()
        val theta: Float = MathUtils.PI2 * u // azimuthal angle
        val phi = java.lang.Math.acos(2f * v - 1f.toDouble()) as Float // polar angle
    ...
    override fun len(): Float {
        return java.lang.Math.sqrt(x * x + y * y + (z * z).toDouble())
    }
    ...
    override fun dst(vector: Vector3): Float {
        ...
        val b = vector.y - y
        val c = vector.z - z
        return java.lang.Math.sqrt(a * a + b * b + (c * c).toDouble())
    }
    ...
    override fun nor(): Vector3 {
        val len2 = this.len2()
        return if (len2 == 0f || len2 == 1f) this else this.scl(1f / java.lang.Math.sqrt(len2.toDouble()) as Float)
    }
    ...
    fun mul(matrix: Matrix4): Vector3 {
        val l_mat: FloatArray = matrix.`val`
     ...
     * @return This vector for chaining
     */
    fun mul(quat: com.badlogic.gdx.math.Quaternion): Vector3 {
    ...
    fun slerp(target: Vector3, alpha: Float): Vector3 {
        ...
        val theta0 = java.lang.Math.acos(dot.toDouble()) as Float
        // theta = angle between this vector and result
        val theta = theta0 * alpha
        val st = java.lang.Math.sin(theta.toDouble()) as Float
        val tx = target.x - x * dot
        val ty = target.y - y * dot
        val tz = target.z - z * dot
        val l2 = tx * tx + ty * ty + tz * tz
        val dl = st * if (l2 < 0.0001f) 1f else 1f / java.lang.Math.sqrt(l2.toDouble()) as Float
        return scl(java.lang.Math.cos(theta.toDouble()) as Float).add(tx * dl, ty * dl, tz * dl).nor()
    }
    ...
    fun fromString(v: String): Vector3 {
        ...
        if (s0 != -1 && s1 != -1 && v[0] == '(' && v[v.length - 1] == ')') {
            try {
                ...
                val z = v.substring(s1 + 1, v.length - 1).toFloat()
    ...
    override fun epsilonEquals(other: Vector3, epsilon: Float): Boolean {
        ...
        if (java.lang.Math.abs(other.y - y) > epsilon) return false
        return if (java.lang.Math.abs(other.z - z) > epsilon) false else true
    }
    /**
    ...
    fun epsilonEquals(x: Float, y: Float, z: Float, epsilon: Float = MathUtils.FLOAT_ROUNDING_ERROR): Boolean {
        ...
        if (java.lang.Math.abs(y - this.y) > epsilon) return false
        return if (java.lang.Math.abs(z - this.z) > epsilon) false else true
    }
