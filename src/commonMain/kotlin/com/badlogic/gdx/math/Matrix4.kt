...
class Matrix4 : Serializable {
    ...
    fun setToProjection(near: Float, far: Float, fovy: Float, aspectRatio: Float): Matrix4 {
        idt()
        val l_fd = (1.0 / java.lang.Math.tan(fovy * (java.lang.Math.PI / 180) / 2.0)) as Float
    ...
    fun lerp(matrix: Matrix4, alpha: Float): Matrix4 {
        for (i in 0..15) values[i] = values[i] * (1 - alpha) + matrix.values[i] * alpha
        return this
    }
    ...
    fun set(mat: Matrix3): Matrix4 {
        values[0] = mat.`val`.get(0)
        values[1] = mat.`val`.get(1)
        values[2] = mat.`val`.get(2)
        values[3] = 0
        values[4] = mat.`val`.get(3)
        values[5] = mat.`val`.get(4)
        values[6] = mat.`val`.get(5)
        values[7] = 0
        values[8] = 0
        values[9] = 0
        values[10] = 1
        values[11] = 0
        values[12] = mat.`val`.get(6)
        values[13] = mat.`val`.get(7)
        values[14] = 0
        values[15] = mat.`val`.get(8)
        return this
    }
     ...
     */
    val scaleX: Float
        get() = if (MathUtils.isZero(values[M01]) && MathUtils.isZero(values[M02])) java.lang.Math.abs(values[M00]) else java.lang.Math.sqrt(scaleXSquared.toDouble())

    /**
     * @return the scale factor on the Y axis (non-negative)
     */
    val scaleY: Float
        get() = if (MathUtils.isZero(values[M10]) && MathUtils.isZero(values[M12])) java.lang.Math.abs(values[M11]) else java.lang.Math.sqrt(scaleYSquared.toDouble())

    /**
     * @return the scale factor on the X axis (non-negative)
     */
    val scaleZ: Float
        get() = if (MathUtils.isZero(values[M20]) && MathUtils.isZero(values[M21])) java.lang.Math.abs(values[M22]) else java.lang.Math.sqrt(scaleZSquared.toDouble())
    ...
    fun hasRotationOrScaling(): Boolean {
        return !(MathUtils.isEqual(values[M00], 1) && MathUtils.isEqual(values[M11], 1) && MathUtils.isEqual(values[M22], 1)
    ...
    companion object {
        ...
        var quat2: Quaternion = Quaternion()
        val l_vez = Vector3()
        val l_vex = Vector3()
        val l_vey = Vector3()
        val tmpVec = Vector3()
