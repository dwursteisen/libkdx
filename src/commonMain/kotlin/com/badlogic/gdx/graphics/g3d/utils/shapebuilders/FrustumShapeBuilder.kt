 ...
 * @author realitix
 */
object FrustumShapeBuilder : BaseShapeBuilder {
     ...
     * @param camera  Camera
     */
    fun build(builder: MeshPartBuilder?, camera: Camera?) {
     ...
     * @param crossColor
     */
    fun build(builder: MeshPartBuilder?, camera: Camera?, frustumColor: Color?, coneColor: Color?, upColor: Color?,
              targetColor: Color?, crossColor: Color?) {
        val planePoints: Array<Vector3?> = camera.frustum.planePoints
        ...
        // Up triangle
        val halfNearSize: Float = tmpV0.set(planePoints[1]).sub(planePoints[0]).scl(0.5f).len()
     ...
     * @param crossColor
     */
    fun build(builder: MeshPartBuilder?, frustum: Frustum?, frustumColor: Color?, crossColor: Color?) {
        val planePoints: Array<Vector3?> = frustum.planePoints
     ...
     */
    private fun middlePoint(point0: Vector3?, point1: Vector3?): Vector3? {
     ...
     */
    private fun centerPoint(point0: Vector3?, point1: Vector3?, point2: Vector3?): Vector3? {
        tmpV0.set(point1).sub(point0).scl(0.5f)
