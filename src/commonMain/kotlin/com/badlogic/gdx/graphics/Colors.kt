 ...
 * `BitmapFontCache` class uses this class to retrieve colors and the user can define his own colors.
 *
 * @author davebaol
 */
object Colors {

    private val map: ObjectMap<String?, Color?>? = ObjectMap<String?, Color?>()

    /**
     * Returns the color map.
     */
    val colors: ObjectMap<String?, Color?>?
        get() = map

    /**
     * Convenience method to lookup a color by `name`. The invocation of this method is equivalent to the expression
     * `Colors.getColors().get(name)`
     *
     * @param name the name of the color
     * @return the color to which the specified `name` is mapped, or `null` if there was no mapping for `name`
     * .
     */
    ...
    /**
     * Convenience method to add a `color` with its `name`. The invocation of this method is equivalent to the
     * expression `Colors.getColors().put(name, color)`
     *
     * @param name  the name of the color
     * @param color the color
     * @return the previous `color` associated with `name`, or `null` if there was no mapping for `name`
     * .
     */
    fun put(name: String?, color: Color?): Color? {
