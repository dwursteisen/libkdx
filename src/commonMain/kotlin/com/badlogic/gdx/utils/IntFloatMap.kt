 ...
 * @author Nathan Sweet
 */
class IntFloatMap @JvmOverloads constructor(initialCapacity: Int = 51, loadFactor: Float = 0.8f) : Iterable<IntFloatMap.Entry?> {
     ...
     */
    constructor(map: IntFloatMap) : this(java.lang.Math.floor(map.capacity * map.loadFactor.toDouble()) as Int, map.loadFactor) {
    ...
    private fun push(insertKey: Int, insertValue: Float, index1: Int, key1: Int, index2: Int, key2: Int, index3: Int, key3: Int) {
    ...
    private fun putStash(key: Int, value: Float) {
    ...
    private fun getStash(key: Int, defaultValue: Float): Float {
        val keyTable = keyTable
        var i = capacity
        val n = i + stashSize
        while (i < n) {
     ...
     * put into the map.
     */
    fun getAndIncrement(key: Int, defaultValue: Float, increment: Float): Float {
        if (key == 0) {
    ...
    private fun getAndIncrementStash(key: Int, defaultValue: Float, increment: Float): Float {
        val keyTable = keyTable
        var i = capacity
        val n = i + stashSize
        while (i < n) {
    ...
    fun remove(key: Int, defaultValue: Float): Float {
        if (key == 0) {
    ...
    fun removeStash(key: Int, defaultValue: Float): Float {
        val keyTable = keyTable
        var i = capacity
        val n = i + stashSize
        while (i < n) {
     ...
     * be an expensive operation.
     */
    fun containsValue(value: Float, epsilon: Float): Boolean {
        if (hasZeroValue && java.lang.Math.abs(zeroValue - value) <= epsilon) return true
        val valueTable = valueTable
        var i = capacity + stashSize
        while (i-- > 0) {
            if (java.lang.Math.abs(valueTable[i] - value) <= epsilon) return true
        }
        return false
    }
     ...
     */
    fun findKey(value: Float, notFound: Int): Int {
    ...
    override fun hashCode(): Int {
        var h = 0
        if (hasZeroValue) {
            h += java.lang.Float.floatToIntBits(zeroValue)
        }
        val keyTable = keyTable
        val valueTable = valueTable
        var i = 0
    ...
    override fun equals(obj: Any?): Boolean {
        ...
        if (obj !is IntFloatMap) return false
        val other = obj
    ...
    class MapIterator(val map: IntFloatMap) {
        var hasNext = false
        var nextIndex = 0
        var currentIndex = 0
        var valid = true
    ...
    class Entries(map: IntFloatMap) : MapIterator(map), Iterable<Entry?>, Iterator<Entry?> {
    ...
    class Values(map: IntFloatMap) : MapIterator(map) {
    ...
    class Keys(map: IntFloatMap) : MapIterator(map) {
        operator fun hasNext(): Boolean {
            if (!valid) throw GdxRuntimeException("#iterator() cannot be used nested.")
            return hasNext
        }
