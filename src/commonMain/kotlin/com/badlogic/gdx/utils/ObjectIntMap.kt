 ...
 * @author Nathan Sweet
 */
class ObjectIntMap<K> @JvmOverloads constructor(initialCapacity: Int = 51, loadFactor: Float = 0.8f) : Iterable<ObjectIntMap.Entry<K>?> {
     ...
     */
    constructor(map: ObjectIntMap<out K>) : this(java.lang.Math.floor(map.capacity * map.loadFactor.toDouble()) as Int, map.loadFactor) {
    ...
    fun putAll(map: ObjectIntMap<out K>) {
    ...
    private fun push(insertKey: K?, insertValue: Int, index1: Int, key1: K, index2: Int, key2: K, index3: Int, key3: K) {
    ...
    private fun putStash(key: K?, value: Int) {
    ...
    private fun getStash(key: K, defaultValue: Int): Int {
        val keyTable: Array<K> = keyTable
        var i = capacity
        val n = i + stashSize
        while (i < n) {
    ...
    private fun getAndIncrementStash(key: K, defaultValue: Int, increment: Int): Int {
        val keyTable: Array<K> = keyTable
        var i = capacity
        val n = i + stashSize
        while (i < n) {
    ...
    fun remove(key: K, defaultValue: Int): Int {
        val hashCode = key.hashCode()
        var index = hashCode and mask
        if (key == keyTable[index]) {
            keyTable[index] = null
    ...
    fun removeStash(key: K, defaultValue: Int): Int {
        val keyTable: Array<K> = keyTable
        var i = capacity
        val n = i + stashSize
        while (i < n) {
    ...
    override fun equals(obj: Any?): Boolean {
        ...
        if (obj !is ObjectIntMap<*>) return false
        val other: ObjectIntMap<K> = obj
    ...
    class MapIterator<K>(map: ObjectIntMap<K>) {
        var hasNext = false
        val map: ObjectIntMap<K?>
        var nextIndex = 0
        var currentIndex = 0
        var valid = true
        fun reset() {
    ...
    class Entries<K>(map: ObjectIntMap<K>) : MapIterator<K>(map), Iterable<Entry<K>?>, MutableIterator<Entry<K>?> {
    ...
    class Values(map: ObjectIntMap<*>) : MapIterator<Any?>(map as ObjectIntMap<Any>) {
        ...
        operator fun next(): Int {
            if (!hasNext) throw NoSuchElementException()
            if (!valid) throw GdxRuntimeException("#iterator() cannot be used nested.")
            val value: Int = map.valueTable.get(nextIndex)
            currentIndex = nextIndex
            findNextIndex()
            return value
        }
    ...
    class Keys<K>(map: ObjectIntMap<K>) : MapIterator<K>(map), Iterable<K>, MutableIterator<K> {
        override fun hasNext(): Boolean {
            if (!valid) throw GdxRuntimeException("#iterator() cannot be used nested.")
            return hasNext
        }
