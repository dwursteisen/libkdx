 ...
 * @author Nathan Sweet
 */
class IntMap<V> @JvmOverloads constructor(initialCapacity: Int = 51, loadFactor: Float = 0.8f) : Iterable<IntMap.Entry<V>?> {

    var size = 0
    var keyTable: IntArray
    var valueTable: kotlin.Array<V?>
    var capacity: Int
    var stashSize = 0
     ...
     */
    constructor(map: IntMap<out V>) : this(java.lang.Math.floor(map.capacity * map.loadFactor.toDouble()) as Int, map.loadFactor) {
    ...
    private fun push(insertKey: Int, insertValue: V?, index1: Int, key1: Int, index2: Int, key2: Int, index3: Int, key3: Int) {
    ...
    private fun putStash(key: Int, value: V?) {
    ...
    private fun getStash(key: Int, defaultValue: V?): V? {
        val keyTable = keyTable
        var i = capacity
        val n = i + stashSize
        while (i < n) {
    ...
    private fun resize(newSize: Int) {
        ...
        valueTable = arrayOfNulls<Any>(newSize + stashCapacity) as kotlin.Array<V?>
        val oldSize = size
    ...
    override fun hashCode(): Int {
        ...
        val keyTable = keyTable
        val valueTable: kotlin.Array<V> = valueTable
        var i = 0
    ...
    class MapIterator<V>(map: IntMap<V>) {
        var hasNext = false
        val map: IntMap<V?>
        var nextIndex = 0
        var currentIndex = 0
    ...
    class Entries<V>(map: IntMap<*>) : MapIterator<V>(map), Iterable<Entry<V>?>, Iterator<Entry<V>?> {
    ...
    class Values<V>(map: IntMap<V>) : MapIterator<V>(map), Iterable<V>, Iterator<V> {
        override fun hasNext(): Boolean {
            if (!valid) throw GdxRuntimeException("#iterator() cannot be used nested.")
            return hasNext
        }
         ...
         */
        fun toArray(): Array<V> {
            val array: Array<*> = Array<Any?>(true, map.size)
    ...
    class Keys(map: IntMap<*>) : MapIterator<Any?>(map) {
    ...
    init {
        ...
        valueTable = arrayOfNulls<Any>(keyTable.size) as kotlin.Array<V?>
    }
}
