...
class Select {
    ...
    fun <T> selectIndex(items: Array<T>, comp: Comparator<T>, kthLowest: Int, size: Int): Int {
        if (size < 1) {
            throw GdxRuntimeException("cannot select from empty array (size < 1)")
        } else if (kthLowest > size) {
            throw GdxRuntimeException("Kth rank is larger than size. k: $kthLowest, size: $size")
        }
        val idx: Int
        ...
        if (kthLowest == 1) {
            ...
            if (quickSelect == null) quickSelect = QuickSelect()
