...
/**
 * This is a near duplicate of [TimSort], modified for use with arrays of objects that implement [Comparable], instead
 ...
 * conjunction with a comparator that simply returns `((Comparable)first).compareTo(Second)`. If this is the case, you are
...
internal class ComparableTimSort {
    ...
    constructor() {
        tmp = arrayOfNulls(INITIAL_TMP_STORAGE_LENGTH)
        runBase = IntArray(40)
        runLen = IntArray(40)
    }

    fun doSort(a: Array<Any?>, lo: Int, hi: Int) {
        var lo = lo
        stackSize = 0
     ...
     */
    private constructor(a: Array<Any?>) {
        this.a = a

        // Allocate temp storage (which may be increased later if necessary)
        val len = a.size
        val newArray = arrayOfNulls<Any>(if (len < 2 * INITIAL_TMP_STORAGE_LENGTH) len ushr 1 else INITIAL_TMP_STORAGE_LENGTH)
        tmp = newArray
    ...
    private fun mergeAt(i: Int) {
         ...
         * place).
         */
        val k = gallopRight(a!![base2] as Comparable<Any?>?, a, base1, len1, 0)
        if (DEBUG) assert(k >= 0)
        base1 += k
        len1 -= k
        if (len1 == 0) return
         ...
         * place).
         */len2 = gallopLeft(a!![base1 + len1 - 1] as Comparable<Any?>?, a, base2, len2, len2 - 1)
    ...
    private fun mergeLo(base1: Int, len1: Int, base2: Int, len2: Int) {
        ...
        var minGallop = minGallop // Use local variable for performance
        outer@ while (true) {
             ...
             */do {
                ...
                if ((a[cursor2] as Comparable<*>?)!!.compareTo(tmp[cursor1]) < 0) {
                    a[dest++] = a[cursor2++]
                    count2++
                    count1 = 0
             ...
             */do {
                if (DEBUG) assert(len1 > 1 && len2 > 0)
                count1 = gallopRight(a[cursor2] as Comparable<*>?, tmp, cursor1, len1, 0)
                if (count1 != 0) {
                ...
                a[dest++] = a[cursor2++]
                if (--len2 == 0) break@outer
                count2 = gallopLeft(tmp[cursor1] as Comparable<*>?, a, cursor2, len2, 0)
                if (count2 != 0) {
    ...
    private fun mergeHi(base1: Int, len1: Int, base2: Int, len2: Int) {
        ...
        outer@ while (true) {
             ...
             */do {
                ...
                if ((tmp[cursor2] as Comparable<*>?)!!.compareTo(a[cursor1]) < 0) {
                    a[dest--] = a[cursor1--]
                    count1++
                    count2 = 0
             ...
             */do {
                if (DEBUG) assert(len1 > 0 && len2 > 1)
                count1 = len1 - gallopRight(tmp[cursor2] as Comparable<*>?, a, base1, len1, len1 - 1)
                if (count1 != 0) {
                    dest -= count1
                    cursor1 -= count1
                    len1 -= count1
                ...
                a[dest--] = tmp[cursor2--]
                if (--len2 == 1) break@outer
                count2 = len2 - gallopLeft(a[cursor1] as Comparable<*>?, tmp, 0, len2, len2 - 1)
                if (count2 != 0) {
                    dest -= count2
                    cursor2 -= count2
                    len2 -= count2
     ...
     */
        @JvmOverloads
        fun sort(a: Array<Any?>, lo: Int = 0, hi: Int = a.size) {
            var lo = lo
            rangeCheck(a.size, lo, hi)
            var nRemaining = hi - lo
            if (nRemaining < 2) return  // Arrays of size 0 and 1 are always sorted
         ...
         */
        private fun binarySort(a: Array<Any?>, lo: Int, hi: Int, start: Int) {
            var start = start
            if (DEBUG) assert(lo <= start && start <= hi)
            if (start == lo) start++
            while (start < hi) {
                val pivot: Comparable<Any?>? = a[start] as Comparable<*>?
         ...
         */
        private fun countRunAndMakeAscending(a: Array<Any?>, lo: Int, hi: Int): Int {
            if (DEBUG) assert(lo < hi)
            var runHi = lo + 1
            if (runHi == hi) return 1

            // Find end of run, and reverse range if descending
            if ((a[runHi++] as Comparable<*>?)!!.compareTo(a[lo]) < 0) { // Descending
                while (runHi < hi && (a[runHi] as Comparable<*>?)!!.compareTo(a[runHi - 1]) < 0) runHi++
         ...
         */
        private fun gallopLeft(key: Comparable<Any?>?, a: Array<Any?>?, base: Int, len: Int, hint: Int): Int {
         ...
         */
        private fun gallopRight(key: Comparable<Any?>?, a: Array<Any?>?, base: Int, len: Int, hint: Int): Int {
            if (DEBUG) assert(len > 0 && hint >= 0 && hint < len)
            var ofs = 1
            var lastOfs = 0
            if (key!!.compareTo(a!![base + hint]) < 0) {
